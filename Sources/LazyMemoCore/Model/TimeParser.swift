...
enum TimeParser {
    ...
    private static func colonClock(_ text: String) -> Result? {
        guard let match = text.firstMatch(of: /(?i)(?:\bat\s+)?\b(\d{1,2}):(\d{2})\b/),
    ...
    private static func bareHour(_ text: String) -> Result? {
        guard let match = text.firstMatch(of: /(?i)\bat\s+(\d{1,2})\b/),
              let stated = Int(match.1), (0...23).contains(stated)
        else { return nil }
    ...
    private static func dressed(
        ...
        guard let part = period(before: clock.startIndex, in: text) else {
            return Result(hour: byConvention(stated), minute: minute, phrases: [String(clock)])
        }
        return Result(
    ...
    static func momentFromNow(
        ...
        if let found = counted(in: text, /(\d{1,3})\s*(?:분|分鐘|分钟|分)\s*(?:뒤|후|後|后|이후|以後|以后)/)
