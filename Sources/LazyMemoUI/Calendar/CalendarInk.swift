...
enum InkBleed {
    ...
    ///
    /// 0.30 에서 내렸다. 고른 날의 밑줄이 숫자에 바짝 붙어 있어서, 얼룩이
    /// 그 바로 아래에 앉으면 둘이 한 덩어리로 뭉쳤다 — 밑줄도 얼룩도 아닌
    /// **초승달 모양의 얼룩덜룩한 것**이 되고, 그건 표시가 아니라 고장으로
    /// 읽힌다 (실제로 고른 날마다 그렇게 보였다).
    static let drop: CGFloat = 0.34
    /// 세로로 눌린 정도. 종이결을 따라 옆으로 번지는 것이 잉크의 성질이다.
    /// 너무 납작하면 얼룩이 아니라 **긁힌 자국**으로 보인다.
    ///
    /// 0.74 는 지나쳤다. 작은 자국일수록 납작한 것이 먼저 눈에 띄어서, 점이
    /// 아니라 **번진 줄**로 보였다.
    static let squash: CGFloat = 0.86

    /// 얼룩의 반지름. 칸의 짧은 변을 기준으로 잡아 5주 달과 6주 달에서
    /// 같은 비율로 보이게 한다.
    ///
    /// 한 건과 네 건은 나란히 놓고도 갈려야 한다 — 지름 차이가 30% 뿐이던
    /// 판에서는 두 칸이 같아 보였고, 그러면 얼룩은 "이 날은 붐빈다" 를 말하지
    /// 못한 채 종이에 묻은 자국으로만 남는다 (`legibleSpan`).
    ///
    /// **그런데 전체가 너무 컸다.** 칸의 짧은 변의 1/3 까지 자라는 자국을
    /// 옅은 색으로 흐리게 칠하면 그것은 잉크가 아니라 **초점이 안 맞은
    /// 사진**으로 보인다 — 달을 통째로 보면 칸마다 먼지가 앉은 것 같았다.
    /// 지금은 펜촉이 한 번 닿은 만큼이고, 대신 심이 또렷하다 (`InkBleedLayer`).
    /// 벌어지는 비율은 그대로 두고 크기만 줄였다.
    static func radius(cell: CGSize, spread: Double) -> CGFloat {
        min(cell.width, cell.height) * CGFloat(0.05 + 0.09 * spread)
    }
