...
/// 맥과 같은 종이·잉크·포레스트 (docs/VISUAL_DESIGN.md). 값은 `LazyMemoUI/Views/Theme.swift`
/// 와 같고, 외관에 따라 갈리는 색만 `UIColor` 로 옮겼다.
enum Paper {
    /// 종이 바탕. 라이트는 미색, 다크는 따뜻한 숯색.
    static let surface = Color(uiColor: UIColor { traits in
        traits.userInterfaceStyle == .dark
            ? UIColor(red: 0.137, green: 0.129, blue: 0.118, alpha: 1)
            : UIColor(red: 0.980, green: 0.969, blue: 0.949, alpha: 1)
    })

    /// 종이 위의 잉크.
    static let ink = Color(uiColor: UIColor { traits in
        traits.userInterfaceStyle == .dark
            ? UIColor(red: 0.902, green: 0.886, blue: 0.855, alpha: 1)
            : UIColor(red: 0.161, green: 0.149, blue: 0.129, alpha: 1)
    })

    static var fadedInk: Color { ink.opacity(0.52) }
}
...
enum Theme {
    ...
    /// 앱이 대신 읽어 준 날짜의 글자색. 라이트에서는 잉크 쪽으로 가라앉힌 호박색.
