...
`앱을 「열어서」 쓰지 않습니다. 메모는 늘 바탕화면에 떠 있고, ⌥⌘N 한 번으로 새 메모가 시작됩니다.`
...
1. 나의 앱 → ＋ → 새로운 앱: 플랫폼 iOS·macOS 둘 다 체크, 이름·기본 언어·번들 ID·SKU·사용자 접근.
2. 앱 정보: 카테고리·저작권·개인정보 처리방침 URL·연령 등급 설문.
3. 가격 및 사용 가능 여부: 무료, 모든 국가.
4. 앱 개인정보: 「데이터를 수집하지 않음」→ 게시.
5. 각 플랫폼 버전 페이지: 스크린샷·부제·홍보 문구·설명·키워드·지원 URL·마케팅 URL·심사 메모·연락처. 빌드는 `archive.sh` / `archive.sh mac` 이 올린 것을 고른다.
6. TestFlight 내부 테스터로 설치해 본 뒤 심사 제출.
