...
`glassEffect` 를 쓰지 않는다. 메모에서, 흐름에서, 빠른 입력에서 차례로 시도했다가 **모두 되돌렸다.** 반투명한 면 위의 글은 씻겨 나간다. 바탕화면 사진이 무엇이든 글은 읽혀야 하는데 유리는 그 통제권을 배경에 넘기고, 빠른 입력처럼 "지금 치고 있는 글자"가 있는 곳에서는 더더욱 그렇다.

떠 있다는 느낌은 투명도가 아니라 **그림자와 크기와 자리**가 만든다. 재질이 하나면 화면 전체가 한 물건으로 읽히기도 한다.
...
마커(`**`, `#`, `[]()`)는 지우지 않고 **흐리게 눌러 둔다.** 지우면 커서가 어디 있는지 알 수 없어진다. 다만 `](https://…)` 처럼 긴 마커는 색만 죽여서는 여전히 시끄러워 글자 크기까지 줄인다 — 링크 이름만 남은 것처럼 보인다.
