...
- **앱 안에서 지우는 길은 메모 오른쪽 버튼 메뉴 하나다.** 겹쳐 뜨는 조작 줄에 휴지통을 두면 「치우기(×)」와 나란히 서서 잘못 누르게 된다 — 둘은 결과가 전혀 다른데 생김새가 닮는다. 지우기는 자주 하는 일이 아니므로 오른쪽 버튼 뒤가 맞고, 항목에 "되돌릴 수 있습니다"를 함께 적어 누르기 **전에** 안심시킨다.
...
- **확정한 메모는 포커스를 뺏지 않는다.** 새 메모를 만들면 종이가 잠깐 앞으로 나왔다 스스로 내려앉고, 키보드는 사용자가 하던 앱에 그대로 있다. 활성화해 버리면 바탕화면 레벨의 **보이지 않는 창**으로 입력이 넘어가 이어서 친 글자가 사라진다. 반대로 아무 표시도 안 하면 "적히긴 한 건가"가 남는다 — 잠깐 보였다 내려앉는 것이 둘 다 피하는 유일한 길이다. 검색 결과를 골라 연 경우는 사용자가 "그 메모를 보자"고 한 것이므로 앞으로 데려온다.
