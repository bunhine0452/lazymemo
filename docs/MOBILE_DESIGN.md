...
단추를 누르면 펜은 비지만 목록은 **읽는 동안 그대로** 선다 — 머리글 「「치과 언제였지?」 · 이 중에서 읽는 중」(맥 E-1 「목록 그대로」). 단추 자리에는 획과 단계의 말(「웹에서 찾는 중」→「5줄 골라 읽는 중」→「답을 적는 중」)이 서고 글 칸의 테두리가 강조색으로 숨 쉰다 — 맥 상자와 같은 잉크(`ThinkingInk`, DESIGN §9.4 다섯째). 비었다가 근거로 줄어드는 두 번의 뜀이 기다리는 사람을 흔들기 때문이다. 답 카드는 무엇을 물었는지를 첫 줄에 든다 — 맥은 상자 안에 답이 서지만 폰의 답은 목록 위에 혼자 선다. 비서가 「어느 메모?」에 후보를 못 붙였으면(「금요일 10시에 다시 알려줘」는 어느 메모의 낱말도 아니다) 지금 목록이 곧 후보다. 답·결과·되물음은 「지금」 띠보다 위에 선다.

첫 실행 안내의 첫 장에 한 줄을 더했다: 「물으면 메모가 답하고, 시키면 합니다 — 전부 이 폰 안에서요」. 맥의 빈 상자 힌트 「적으면 메모, 찾으면 검색, 물으면 답」(D3)의 자리다 — 폰의 펜에는 힌트 줄이 없다. 2026-09-21 에 안내를 다섯 장으로 다시 썼다(펜 · 「지금」과 다시 보기 · 손짓 · 달력 · 앱 밖의 문) — 장마다 앱의 진짜 부품을 작게 그린 그림(`TutorialArt`)이 먼저고, 적는 손짓은 전부 본 화면에 그대로 있는 것만이다.
...
| 오른쪽으로 밂 | 「고정」/「고정 해제」 (Notes 와 같은 방향 → 노트 §7). **일정이 있는 줄은 「미루기」가 앞이다** — 달력 탭(§6)과 같은 낱말·같은 방향, 끝까지 밀면 그것. 같은 메모가 탭마다 다른 손짓을 갖지 않게 (2026-09-17 편의성 감사 §2.1). 「지금」 카드도 같다 |
...
**키보드가 내려가 있을 때 `- [ ]` 를 누르면 뒤집힌다.** 「다 했다」를 알리려고 괄호 안 글자를 고치게 하는 것은 맥의 종이가 이미 거절한 불편이다. 머리 `- [ ] ` 가 누를 수 있는 글 조각(`textItemTag`)이고, 적는 중(키보드가 올라와 있을 때)의 누르기는 커서를 놓는 손이라 뒤집지 않는다 — 맥의 「편집 중이 아닐 때만 링크를 연다」와 같은 갈래. 목록 줄의 ⏎ 도 맥과 같다: `- [ ] 우유` 에서 치면 다음 줄에 빈 상자, 빈 머리에서 치면 목록에서 나온다 (`LazyMemoCore/ListEditing`, 규칙은 한 곳).
...
| 칸 | 44×44 이상. 일정은 숫자 밑의 **점** — 점 하나가 메모 하나이고 **점의 색이 그 메모의 색**이다 (아래 목록의 색 점·위젯과 같은 낱말, `WidgetAgenda.monthInks`). 셋을 넘으면 셋 — 숫자를 세지 않는다 (§10.4). 오늘의 포레스트 원 위에서는 크림 한 색 (2026-09-21) |
| 오늘 | accent 원, 크림 숫자 — 시스템 Calendar 의 채운 원과 같은 문법, 색만 우리 것 |
| 고른 날 | 옅은 원 — 칸에서 칸으로 **미끄러진다** (`matchedGeometryEffect`, `Motion.settle`). 아래 목록과 머리의 날짜가 같은 박자로 갈린다 ("Touch and content should stay together"). Reduce Motion 이면 짧은 페이드 (2026-09-21) |
| 앞뒤 달 | 40%, 누르면 그 달 |
...
|---|---|
| 칸 누름 | 그 날이 아래에 선다 |
| 좌우로 쓸기 (격자) | 달을 넘긴다 — 판이 손가락을 따라오고, 놓으면 흘러갈 자리가 반을 넘겼는지로 넘길지 돌아올지 정한다 (`predictedEndTranslation`). 놓은 뒤의 스프링은 손가락의 속도를 이어받는다 (`Motion.settle(velocity:over:)` — 속도 0 에서 다시 출발하면 놓는 순간 멈칫한다, 2026-09-22). 화살표·「오늘」도 같은 미끄러짐. "Touch and content should stay together and move as one thing" (→ 노트 §2, 803). Reduce Motion 이면 밀지 않고 바꿔 끼운다. 세 판은 달 번호가 정체성이라 다 간 뒤 새로 짓는 것은 새로 보이는 한 판뿐이고, 점은 앞뒤 두 달씩 미리 읽어 넘기는 중에 바뀌는 판이 없다 |
