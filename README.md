...
>
> [소개 페이지](https://bunhine0452.github.io/lazymemo/ko/) ([English](https://bunhine0452.github.io/lazymemo/)) · [릴리스](https://github.com/bunhine0452/lazymemo/releases/latest) · [무엇이 바뀌었나](#이번-판-030)
...
- **처음 켜면 4단계 체험이 뜬다.** 빠른 입력 → 날짜 인식 → 서랍에 넣고 꺼내기 → 시작. 연습한 글은 저장되지 않고, 메뉴 → 「시작하기 및 사용 안내…」로 언제든 다시 본다.
- **크림과 포레스트.** 메모·달력·서랍·빠른 입력·아이콘을 한 결로 다시 그렸다. 회색 도구 대신 또렷한 주요 행동, 둥근 카드, 얕은 그림자 ([docs/VISUAL_DESIGN.md](docs/VISUAL_DESIGN.md)).
- **빠른 입력에 손이 갈 자리가 생겼다.** 확정 버튼, `#사진`·링크·할 일 바로가기, 「최근 메모 N장」 머리글, 닫기. 스무 장으로 펼친 결과는 스크롤 안에 담긴다.
- **서랍에서 찾는다.** 글자를 치면 거르고, `ㅈㅂㄱ` 같은 첫소리로도 찾는다. ⌘-클릭으로 여러 장을 골라 한꺼번에 꺼내거나 지우고, 키보드만으로 훑는다.
- **Tahoe 아이콘.** macOS 26 은 아이콘 실루엣을 스스로 깎는다는 것을 재 보고 알아서, 그릇을 걷어내고 종이로 캔버스를 채웠다.
- **App Store 배포본은 스스로 판을 갈지 않는다.** 영수증이 있으면 GitHub 를 보지 않고 관련 메뉴도 숨긴다 — 아직 제출 빌드는 아니다 ([docs/APP_STORE_READINESS.md](docs/APP_STORE_READINESS.md)).
...
release 번들은 **3.6MB**, 내려받는 zip 은 **1.6MB** 다. `swift build` 가 내놓는 실행 파일의 절반 이상은 디버거가 읽는 심볼 이름표라, 서명 직전에 `strip` 으로 턴다. dSYM 은 `.build` 에 남아 크래시 로그 심볼화는 그대로 된다.
...
| `esc` 또는 `×` | 닫기. **적던 글은 상자가 기억한다** — 다시 열면 그대로 있고 전부 선택돼 있어 그냥 치면 덮어쓴다 |
| 메뉴바 아이콘 **좌클릭** | 빠른 입력 |
...
바탕화면 왼쪽 아래에 **종이 무더기**가 하나 앉아 있다. × 를 눌러 치운 종이와 스스로 물러난 종이가 거기 있다. 폴더가 아니다 — 이름도 없고, 여러 개도 없고, 무엇을 넣을지 정할 필요도 없다. 책상 위의 종이를 한쪽으로 미는 것과 같다.

| | |
|---|---|
| 무더기를 **누르기** | 펼치고, 다시 눌러 접는다. 위에 제목과 몇 장인지, 닫기가 있다 |
| 종이를 **서랍 위로 끌어다 놓기** | 넣는다. 놓기 전에 종이가 비쳐서 어디 놓는지 보인다 |
| 펼친 무더기의 한 장을 **누르기** | 원래 크기로 펼쳐 읽는다. 여기서는 **읽기만 한다** — 고치려면 「꺼내기」 |
| `⌘` **누른 채 누르기** | 여러 장 고르기. 바닥 줄에 「모두 꺼내기 · 모두 지우기」가 뜬다 |
| **글자 치기** (바닥 왼쪽 찾기 줄) | 제목과 본문에서 거른다. **첫소리로도 찾는다** — `ㅈㅂㄱ` 가 「장보기」를 찾는다. 못 찾으면 «비었다» 가 아니라 «N장 중 없다» 고 말한다 |
| `↑` `↓` | 무더기를 훑는다. 끝에서 감아 돌지 않는다 |
| `⏎` / `⌘⏎` / `⌘⌫` | 짚은 장을 펼치기 / 꺼내기 / 지우기 |
| `⌘F` | 찾기 줄로 |
| `esc` | **한 겹씩 되돌린다** — 고르기 → 찾기 → 펼친 종이 → 짚은 자리 → 서랍 |
| 바닥 오른쪽 **「그리고 N장 더」** | 여덟 장씩 더 펼친다. 화면이 허락하는 데까지 |
...
메모 폴더를 iCloud Drive 안으로 옮겨 두면(메뉴 → 설정 → 「메모 폴더 옮기기…」), 아이폰 단축어 하나로 메모가 꽂힌다. **앱이 폴더에 떨어진 낯선 이름의 `.md` 를 알아서 받아 앉히기 때문이다** — 파일 이름을 맞출 필요도, frontmatter 를 적을 필요도 없다.
...
**동기화는 iCloud 가 한다.** 그래서 맥에 나타나기까지 몇 분에서 길게는 한참 걸릴 수 있다 — 급한 것을 던지는 길로는 맞지 않는다. 별도 백엔드가 없다는 것의 값과 대가가 여기서 같이 온다.

## 업데이트

**앱이 스스로 바꾼다.** 메뉴 → 「새 판 0.3.0 이 있습니다」를 누르면 받아서 갈아 끼우고 다시 연다. 화면에서 보이는 것은 앱이 잠깐 닫혔다 뜨는 것뿐이다.
...
| `⌥⌘L` 지금 여기 | 좌표 하나 (주소로 바꾸려고) | `⌥⌘L` 을 누를 때마다 |
| 가면 떠오르기 | **아무것도 안 나간다** | 설정에서 **직접 켤 때만** (기본 꺼짐) |

**메모 본문이 컴퓨터 밖으로 나가는 길은 위의 셋뿐이고, 셋 다 Claude 로 간다.** 그 밖의 것들이 내보내는 것은 주소나 좌표 하나이며 메모는 실리지 않는다.

권한을 묻는 자리는 넷이고, **첫 실행에는 하나도 없다.**
...
- 알림 권한은 **요구하지 않는다.** 시각이 되거나 그 자리에 도착하면 시스템 배너가 아니라 바탕화면의 종이가 앞으로 나온다.
...
**2. 시간이 유일한 구조다.** 게으른 사람은 폴더도 태그도 유지하지 않는다. 그래서 달력이 **만지는 물건**이다 — 일정을 집어 다른 날에 놓고, 한 번 눌러 미룬다. `내일 오후 3시 치과` 라고 적으면 앱이 알아서 날짜를 읽는다. 형식을 배우게 하지 않는다. 서랍의 찾기도 분류를 만들지 않는다 — 거르기일 뿐이고, 글자를 지우면 아무 흔적도 남지 않는다.
...
```sh
./scripts/test.sh                  # 단위 테스트 (649개 · 98 스위트)
...
`.build` 안의 `ModuleCache` 는 상한 없이 자라 혼자 수백 MB 가 된다. 전부 다시 만들어지는 것이라 `./scripts/clean.sh` 로 언제든 털면 된다 — 기본값은 코드 산출물을 남겨 다음 빌드가 증분으로 돈다.
...
| 서랍 | 폴더 하나, 이름 없음. 찾기는 분류를 만들지 않는다 |
