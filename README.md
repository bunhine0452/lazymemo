...
- **테마 여덟 벌, 그리고 당신의 것.** 설정 → 「테마」— 크림과 포레스트(기본)·미드나잇·세피아·흑백 잉크·봄 파스텔·숲속 어둠·바다·장미. 견본을 누르는 순간 바탕화면의 종이가 그 자리에서 바뀐다. 「직접 고르기」로 강조색·종이에 스미는 색·글자 크기·종이 결을 얹고, 「기본으로」 되돌린다. 어느 조합이든 글자는 읽히는 바닥(4.5:1) 위에 서고, 폰과 위젯이 같은 색을 입는다(`settings.json` 이 정본). 색을 세 곳에 따로 적던 것을 한 카탈로그로 모았다 ([docs/VISUAL_DESIGN.md](docs/VISUAL_DESIGN.md) 「테마」).
