...
>
> [소개 페이지](https://bunhine0452.github.io/lazymemo/ko/) ([English](https://bunhine0452.github.io/lazymemo/)) · [릴리스](https://github.com/bunhine0452/lazymemo/releases/latest) · [무엇이 바뀌었나](#이번-판-040)
...
- **서랍에 폴더가 생겼다.** 「장보기」·「읽을 것」·「집」처럼 이름을 짓고 종이를 그리로 넣는다. 폴더는 서랍의 칸이지 자리가 아니다 — 폴더에 넣는 것이 곧 서랍에 넣는 것이고, 꺼낸 종이도 이름표는 그대로다. 이름표는 파일의 `folder:` 한 줄이라 Claude(MCP)도 `list_folders` 로 같은 것을 본다.
- **서랍을 목록으로 다시 그렸다.** 겹친 종이 무더기와 바닥의 작은 찾기 알약을 버렸다. 위에서부터 찾기 줄 → 폴더 띠 → 색·제목·둘째 줄·시각이 있는 목록. 줄에 손을 얹으면 꺼내기·폴더·지우기, 누르면 아래로 펼쳐져 본문. 줄을 칩으로 끌어 옮기고, 넘치면 스크롤한다. 닫힌 서랍은 「서랍 N」 탭 하나다 ([설계문서 §16.11](docs/DESIGN.md)).
- **빠른 입력의 초안이 앱을 껐다 켜도 남는다.** 판을 갈 때 앱이 스스로 닫혔다 뜨므로, 이것이 없으면 「새 판으로 바꾸기」가 초안을 지우는 버튼이었다. 메모가 아니라 초안이라 파일에는 안 쌓인다.
- **빠른 입력도 첫소리로 찾는다.** `ㅈㅂㄱ` 가 「장보기」를 찾는다 — 서랍과 같은 규칙이다.
- **한꺼번에 들어온 메모가 바탕화면에 서지 않는다.** 아이폰 단축어·MCP 로 여러 장이 한 번에 떨어질 때 파일 감시가 도중에 발화하면 잠깐 못 읽은 메모의 좌표가 지워져, 서랍에 있어야 할 종이가 바탕화면에 섰다. 좌표 정리에 10초 유예를 두었다.
- **소개 영상.** 앱이 스스로 한 바퀴 도는 주행 모드(`LAZYMEMO_DEMO`)와 녹화 스크립트. 위의 영상이 그것이다.
...
바탕화면 왼쪽 아래에 **「서랍 N」 탭**이 하나 앉아 있다. × 를 눌러 치운 종이와 스스로 물러난 종이가 거기 있다. 안은 **폴더**로 나뉜다 — 「장보기」·「읽을 것」·「집」처럼 이름을 짓고, 종이를 그리로 넣는다. 폴더는 서랍의 칸이지 자리가 아니다: 폴더에 넣는 것은 곧 서랍에 넣는 것이고, 꺼낸 종이도 이름표는 그대로라 다시 넣으면 같은 칸으로 돌아간다. 이름표는 파일의 `folder:` 한 줄에 적혀 Claude(MCP)도 같은 것을 본다.

| | |
|---|---|
| 탭을 **누르기** | 펼치고, 다시 눌러 접는다. 위에서부터 찾기 줄 · 폴더 띠 · 목록 · 바닥 줄 |
| 종이를 **탭 위로 끌어다 놓기** | 넣는다. 놓기 전에 종이가 비치고 탭이 「놓으면 들어옵니다」라고 말한다. **폴더를 펼쳐 둔 채 놓으면 그 폴더로 간다** |
...
| 줄을 **누르기** | 그 줄이 아래로 펼쳐져 본문을 보인다. **읽기만 한다** — 고치려면 「꺼내기」 |
| `⌘` **누른 채 누르기** | 여러 장 고르기. 바닥 줄에 「모두 꺼내기 · 옮기기 · 지우기」가 뜬다 |
| **글자 치기** (위쪽 찾기 줄) | 제목과 본문에서 거른다. **첫소리로도 찾는다** — `ㅈㅂㄱ` 가 「장보기」를 찾는다. 못 찾으면 «비었다» 가 아니라 «N장 중 없다» 고 말한다 |
...
**앱이 스스로 바꾼다.** 메뉴 → 「새 판 0.4.0 으로 바꾸기」를 누르면 받아서 갈아 끼우고 다시 연다. 화면에서 보이는 것은 앱이 잠깐 닫혔다 뜨는 것뿐이다.
