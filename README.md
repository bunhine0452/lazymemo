...
- **웹에서 찾기** — 메모에 없으면 **「웹에서 찾기」**를 권한다(맥은 빈 상자에서 ⌘↵ 한 번). 「웹에서 서울 내일 날씨」「달러 환율 검색해줘」처럼 말하면 메모를 거치지 않고 바로. 검색은 앱이 DuckDuckGo 에 **키 없이** 하고(질문 한 줄만 나간다, 메모는 안 나간다), 모델은 결과 다섯 줄을 읽어 한두 문장으로 답하며 출처 링크를 단다 — 누르면 브라우저로. 모델이 없으면 결과 셋을 그대로 보여 준다. 답 밑에는 **결과 전부**가 한 칸씩 선다(인용한 것이 앞, 점이 찍힌다). 그리고 **「이걸 어떻게 할까요?」** — 메모로 남기기 · 정리해서 남기기(모델) · 기존 메모에 붙이기. 글로 해도 같다: 「메모해」「정리해줘」「치과 메모에 추가해줘」(이름을 대면 그 메모에, 안 대면 고르게 한다). 남긴 메모에는 답·출처·주소와 「어디서 찾음」 한 줄이 든다. 문서화된 API 가 아니라 DDG 가 막으면 「웹에 닿지 못했습니다」다(가는 길의 네이버 지도와 같은 결정 — 키가 필요한 검색은 두지 않는다).
