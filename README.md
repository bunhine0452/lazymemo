...
대중교통은 **네이버 지도 웹**이 브라우저에서 쓰는 길찾기가 키 없이 답한다 — 문서화된 API 가 아니라 네이버가 막으면 끊길 수 있고, 그때는 설정에 넣어 둔 ODsay 키(선택, `lab.odsay.com` 무료)로 넘어가며, 둘 다 없으면 택시만 잰다. 택시는 애플 지도의 자동차 길에 서울 요금표를 댄 어림이다. 자리 이름도 네이버 지도의 검색이 먼저고 애플 지도가 예비다. 묻지 않게 하려면 설정의 「약속을 적으면 가는 길 묻기」를 끈다.
...
| 약속의 가는 길 묻기 (맥·폰) | 출발지·약속 자리의 **이름과 좌표** — 네이버 지도(자리 검색·짧은 링크 풀기·대중교통 길찾기), 애플 지도(택시), 키를 넣었을 때만 ODsay | 약속을 적은 뒤 「어디서 출발하시나요?」에 **답할 때만**. 「됐어」면 아무것도 안 나간다 · 설정에서 묻지 않게 끌 수 있다 · 메모 본문은 나가지 않는다 |
