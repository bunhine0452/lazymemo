...
폰에는 바탕화면이 없으므로 종이도 서랍도 없다. 있는 것은 **펜**(바닥, 적기와 찾기), **무더기**(한 목록), **달력**이다. 화면의 규칙은 [`docs/MOBILE_DESIGN.md`](docs/MOBILE_DESIGN.md) 에 있다. 다른 앱에서 글을 고르고 공유 → 「lazymemo 에 적기」가 맥의 서비스 메뉴와 같은 문이고, 펜의 핀은 **누를 때만** 지금 자리를 재어 붙인다. 종이의 체크상자 `- [ ]` 는 **키보드가 내려가 있을 때 누르면** 뒤집히고(적는 중의 누르기는 커서를 놓는다), 목록 줄의 ⏎ 는 맥과 같이 머리를 잇는다.
