...
그 시각에 맥은 종이를 앞으로 꺼낸다. **시스템 알림은 켠 기기에서만** — 「이 기기에서 알림 받기」를 켜면 그때 한 번 권한을 묻고, 그 뒤로 일정 시각과 다시 볼 시각에 알림이 오며 누르면 그 메모가 열린다. **배너에 단추가 있다** — 길게 누르면(맥은 배너 위에 포인터) 「한 시간 뒤」「내일 아침 9시」「봤어요」, 출발 알림이면 「10분 뒤」「지도 열기」. 앱을 열지 않고 배너에서 끝나고, 미룬 시각은 파일에 적혀 다른 기기도 같은 것을 본다. 「봤어요」는 폰의 「지금」 띠와 위젯에서 그 카드를 내리고 맥은 나온 종이를 내린다. 잠금 화면에 메모 제목이 보인다는 것을 켜기 전에 적어 둔다. 날짜만 있는 메모에는 아침 알림을 지어내지 않고, 지운 것·치워 둔 것·다 체크한 목록은 걸지 않는다. 알림은 이 앱이 파일을 읽은 기기에서 걸리므로 — 폰이 꺼진 동안 맥에서 적은 것은 폰이 다시 읽은 뒤에 걸린다. 양쪽에서 켜면 양쪽에서 울릴 수 있고, 집중 모드와 시스템 설정에 따라 전달이 달라진다. 서버가 없으니 기기 간 알림 동기화를 약속하지 않는다.

폰의 목록 위에는 **「지금」** 최대 세 장 — 오늘 다시 볼 것·오늘 일정·고정한 것 중에서 **다가오는 시각부터**, 그다음 방금 지난 것, 그다음 시각이 없는 것. 이유와 시각이 함께 적히고, 띠에 오른 것은 아래 「나머지」 목록에서 빠진다. 「봤어요」로 내려놓으면 이 기기에서만 사라지고, 시각을 미루거나 날이 바뀌면 다시 오른다. 일정이 있는 카드와 줄은 **오른쪽으로 밀면 「미루기」** — 달력 탭과 같은 손짓으로 하루 미뤄지고, 흔들면 되돌린다. 찾는 중이거나 폴더를 골랐을 때는 띠가 없다. 계획과 경계는 [docs/RECALL_PLAN.md](docs/RECALL_PLAN.md).
