...
- **아이폰 앱.** `ios/` 의 lazymemo 가 맥과 **같은 메모 파일을 같은 폴더에서** 본다 — 맥의 메뉴 → 설정 → 「iCloud 로 동기화…」 한 번이면 폴더가 iCloud Drive 의 「LazyMemo」로 가고, 폰은 켜는 순간 그것을 찾는다. 폰에는 펜(적기·찾기)·무더기·달력이 있고, 공유 시트의 「lazymemo 에 적기」와 핀(지금 여기)이 맥의 문과 같다 ([아이폰](#아이폰--같은-폴더를-보는-앱)).
- **다시 보기 — 필요할 때 다시 펼쳐 준다.** 종이의 「다시 보기…」로 다시 볼 시각을 정하면 그 시각에 맥은 종이를 앞으로 꺼낸다. 「이 기기에서 알림 받기」를 켠 기기에서만 시스템 알림이 오고(기본 꺼짐, 기기별), 폰의 목록 위에는 오늘 챙길 「지금」 세 장 ([다시 보기](#다시-보기--필요할-때-다시-펼쳐-준다)).
- **이 기기의 비서 (미리보기).** 빠른 입력 상자와 폰의 펜이 비서를 겸한다 — 「치과 언제였지?」라고 물으면 메모를 읽고 근거와 함께 답하고, 「금요일 10시에 다시 알려줘」라고 시키면 무엇을 바꿀지 보여 준 뒤 적용한다. 대화로 일정도 만든다. 모델(Gemma 4 E2B, 2.6GB)을 한 번 받으면 그 뒤로는 인터넷 없이 돈다 — 메모는 아무것도 밖으로 나가지 않는다 ([비서](#이-기기의-비서-선택-미리보기)). **그래서 번들이 3.6MB 에서 70MB 가 됐다** — 추론 엔진이 들어 있다. 모델은 따로 받을 때만 온다.
- **Spotlight 에서 찾기.** 메모가 시스템 검색에 오른다 — 앱을 열지 않고도 ⌘Space 에 제목이나 글 한 조각을 치면 나오고, 누르면 그 메모가 열린다. 맥·폰 둘 다, 기본 켜짐, 아무것도 밖으로 나가지 않는다. 맥은 설정에서 끌 수 있다.
- **자리 카드.** 장소가 붙은 메모의 머리에 지도가 앉고, 「가는 길」은 카카오맵·네이버 지도·애플 지도 중 누른 앱이 연다. 네이버·카카오·구글·애플 지도의 「공유」 텍스트를 어느 문으로 던져도 첫 줄이 장소가 되고, 좌표가 적힌 링크면 좌표까지 읽는다 — 접속 없이 ([어디서든 던지기](#어디서든-던지기)).
- **영어로도 쓴다.** 화면의 말 전부가 `en.lproj` 를 지나고, 날짜는 그 말의 어순으로 적힌다. 기계가 영어면 영어, 한국어면 한국어 — 한국어 앱에 「1 photos」가 서던 것도 이때 고쳤다.
- **「지금」·「당장」을 읽는다.** 날이 아니라 이 순간이라, 시각까지 붙는다.
- **App Store 판** (TestFlight 중). 샌드박스 안이라 Claude 연동과 자체 업데이트가 없고, 기본 자리가 iCloud 의 「LazyMemo」 폴더다. 나머지는 전부 같다.
