...
**이 앱은 아직 공증받지 않았다.** (개발자 계정이 생겨 `build-app.sh` 에 Developer ID 서명·공증 경로는 들어 있다 — `LAZYMEMO_SIGN_IDENTITY`·`LAZYMEMO_NOTARY_PROFILE`. 릴리스가 그 길로 나가는 순간 아래 문단은 지워진다.) 내려받은 앱에는 `com.apple.quarantine` 이 붙고, macOS 15 부터는 우클릭-열기 우회가 사라져 딱지가 붙은 미공증 앱은 「손상되었습니다」로 끝난다. 그래서 cask 가 설치 뒤 그 딱지를 떼어 낸다 — **미봉책이고, Developer ID 를 받는 즉시 사라진다** ([설계문서 §12.2](docs/DESIGN.md)). 믿을 수 없다면 소스에서 빌드하는 쪽이 맞다: 그렇게 만든 앱에는 애초에 딱지가 붙지 않는다.
...
| 폰의 자리 카드 | 장소 **이름** 하나 (좌표로 바꾸려고, 애플 지도에) | 장소가 붙은 메모를 폰에서 **열 때** — 파일에 좌표가 이미 있으면 안 나간다 |
| 폰의 「가는 길」 단추 | 좌표와 이름 — 카카오맵·네이버 지도·애플 지도 중 누른 앱으로 | **누를 때마다**. 출발지는 그 앱이 스스로 잰다 |
