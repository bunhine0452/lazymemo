...
>
> [소개 페이지](https://bunhine0452.github.io/lazymemo/ko/) ([English](https://bunhine0452.github.io/lazymemo/)) · [릴리스](https://github.com/bunhine0452/lazymemo/releases/latest) · [무엇이 바뀌었나](#이번-판--093)
...
- **적은 것을 놓치지 않게 — 제품의 중심을 다시 잡았다.** 바로 남기고, 필요할 때 원문과 함께 다시 보고, 그 자리에서 끝내거나 미룬다 ([인계서](docs/PRODUCT_IMPLEMENTATION_HANDOFF.md)).
- **⌘⏎·「남기기」는 늘 적는다.** 물음표·「알림」·「폴더」가 들어 있어도 메모다 — 모델 없는 기계에서도. 비서에게는 **⌥⌘⏎ / ✦** 로만. 「어디서 출발하시나요?」「약속 시간이 언제인가요?」는 다음 글을 먹지 않고, 적힌 뒤 결과 카드의 「가는 길」「시각 정하기」를 누를 때만 묻는다. 첫 실행은 안내 다섯 장 대신 첫 메모부터 — 적힌 뒤 필요한 조작 하나만 한 줄로.
- **자동 정리가 사람의 뜻을 못 덮는다.** 미래에 다시 보기로 한 지난 일정, 마감이 지난 칸 남은 목록, 도로 꺼낸 것(`kept:`)은 규칙이 치우지 않는다 — 날짜가 지났다고 끝난 것이 아니다. 맥 종이·자리 알림·OS 예약이 한 규칙으로 같은 것을 뺀다.
- **완료와 보관** (`done:`·`archived:`). 줄·카드·달력·종이에서 끝내거나 넣어 두고, 되돌린다. 되풀이하는 일은 이 회차만 끝난 것이라 다음 회차에 도로 산다. 「지금」 세 장 아래 **「놓친 2 · 오늘 5 · 나중에 3」** 이 나머지 전부로 가는 길 — 놓친 것은 날이 바뀌어도 거기 있고, 푸시는 없다. 맥 메뉴도 같은 요약·「보관한 N장」·치워 둔 것 하나씩 꺼내기.
- **저장·예약을 믿을 수 있다.** 초안이 기기에 안 남으면 말하고 다시 시도하며, 맥 상자는 적힌 뒤에 비워 실패해도 글이 남는다. 적은 직후 **알림 영수증** — 「이 기기에 알림 예약됨 · 9/25 9:00」「날짜만 적혀 알림 없음」「알림 꺼짐」「예약 실패 · 다시 시도」처럼 이 기기에서 확인한 사실만, 폰 펜·맥 달력 줄·공유 시트·Siri 가 같은 말로. 공유 시트와 Siri 는 앱과 같은 이름으로 그 자리에서 예약한다.
- **충돌본을 사람이 복구한다.** 두 기기가 따로 고쳐 만난 진 판본은 휴지통에서 「다른 기기의 판」으로 따로 서고, 두 판을 견줘 「이 판으로」「둘 다 남기기」.
- 폰 달력 넘김의 버벅임(한 번에 판 여덟 번 그리던 것)을 고쳤다.

지난 판들(0.6.0 ~ 0.9.4)은 [docs/CHANGELOG.md](docs/CHANGELOG.md) 에 있다.
...
첫 실행에는 안내서 대신 **빠른 입력 상자가 뜬다** — 한 줄 적고 ⌘⏎. 첫 메모가 바탕화면에 서면 필요한 조작 하나(다시 부르는 단축키)만 한 줄로 듣는다. 아래의 「lazymemo 시작하기」 다섯 단계는 메뉴 → **「시작하기 및 사용 안내…」** 에 있다. 읽는 안내가 아니라 **만져 보는 안내**다.

| 단계 | 무엇을 해 보나 |
|---|---|
| 01 · 바로 적기 | 연습용 빠른 입력에 한 줄 적고 「남기기」. 실제 상자에서는 이것이 메모 한 장이 된다. 같은 상자에 치면 첫소리로도 찾는다 |
| 02 · 날짜에 맡기기 | `내일 오후 3시 치과 @강남역` 을 고쳐 써 보면 **진짜 파서**가 무엇을 날짜로 읽었는지 바로 보여 준다. `@자리`·되풀이 낱말도 여기서 |
| 03 · 종이와 서랍 | 종이를 서랍에 넣었다 꺼내 본다 — 실제 종이의 × 나 esc 가 하는 일. `⌥⌘P` 종이 보기, 끝난 것이 스스로 물러나는 규칙 |
| 04 · 달력과 다시 보기 | 종이의 달력 아이콘으로 날짜를 놓고, 칸을 누르고 끌어 옮기고, 우클릭 「다시 보기…」. 알림은 설정에서 켤 때만 |
| 05 · 준비 완료 | **이 컴퓨터에 설정된** 단축키 넷(빠른 입력·클립보드 즉시 메모·종이 보기·지금 여기)과 메뉴바 좌클릭·우클릭, **「로그인할 때 시작」** 체크박스 하나 — 켜는 것은 당신이고 권한은 묻지 않는다. 「첫 메모 적기」 또는 「달력 열기」 |

연습한 글은 어디에도 저장되지 않는다. 「건너뛰고 메모 적기」로 언제든 빠져나올 수 있고, 다시 열 때는 지금 설정된 단축키로 시작한다.

## 쓰는 법

| | |
|---|---|
| `⌥⌘N` | 빠른 입력. **메뉴바 아이콘 밑에 말풍선으로 뜬다.** 치면 기존 메모가 걸러진다 — **첫소리로도 찾는다**, `ㅈㅂㄱ` 가 「장보기」를 찾는다. 단축키는 메뉴 → 설정…(⌘,)에서 바꿀 수 있다 |
| `⌥⌘V` | **클립보드 즉시 메모.** 복사한 글이나 사진을 창을 열지 않고 바탕화면에 바로 새 종이로 꽂는다. 이 단축키도 메뉴 → 설정…에서 바꿀 수 있다 |
| `⌥⌘P` | **종이 보기.** 다른 앱 창 뒤에 눕는 종이를 **전부 잠깐 앞에 세운다** — 손은 하던 앱에 그대로. 몇 초 뒤 스스로 도로 눕고, 그 사이 종이를 누르면 그 종이만 손에 남는다. 서 있는 동안 다시 누르면 곧 내려앉는다. 메뉴 → 「종이 보기」와 같고, 단축키는 설정에서 바꾼다 |
| `⏎` | 다음 줄. 메모는 한 줄로 끝나지 않는다. **목록 줄이면 머리가 이어진다** — `- [ ] 우유` 에서 치면 다음 줄에 빈 상자가 서고, `1.` 뒤엔 `2.`, `- ` 뒤엔 `- `. 더 적을 것이 없어 빈 머리에서 한 번 더 치면 목록에서 나온다. 종이와 폰의 종이도 같다 |
| `⌘⏎` 또는 **「메모 남기기」** | 적기 끝 — **늘 그대로 적는다.** 물음표·「알림」·「폴더」가 들어 있어도 메모다. 날짜가 읽혔으면 버튼이 「달력에 남기기」로 바뀐다. ↑↓ 로 찾은 메모를 고른 상태면 그 메모를 연다. 적힌 뒤 권할 것(시각 정하기·가는 길 찾기)이 있으면 상자에 **결과 카드**가 서고 다음 글을 받는다 — 누르면 그때 묻는다. 적지 못하면 글은 그대로 남고 까닭이 한 줄 선다 |
| `⌥⌘⏎` 또는 **✦** | **비서에게.** 「치과 언제였지?」면 메모가 답하고, 「금요일 10시에 다시 알려줘」면 시키고, 「웹에서 …」면 웹. 모델이 없으면 단추도 없다 — 적기는 모델 없이 산다 |
...
| 상자 **바깥을 누르기** | 들고 있는 것이 없으면 닫힌다. **답을 기다리거나 답·결과·되물음이 서 있으면 닫히지 않는다** — 상자는 그 자리에 남고 손만 누른 곳으로 간다. 브라우저에서 할 일을 하고 단축키나 상자 클릭으로 돌아오면 답이 거기 있다. 닫는 것은 그때 `esc`·`×` 다 |
| 메뉴바 아이콘 **좌클릭** | 빠른 입력 |
...
| 종이에서 `⌘F` | **찾기 줄.** 긴 메모에서 낱말을 찾는다 — 종이 머리에 서고, 치는 대로 짚는다. `esc` 로 접는다 |

### 달력 — 만지는 물건

메모와 일정을 따로 만들지 않는다. `due`(날짜) 또는 `at`(시각) 칸이 채워진 메모가 곧 일정이고, 빠른 입력에 `내일 3시` 라고 적거나 종이의 달력 아이콘으로 놓거나 Claude 가 MCP 로 고치거나 — **어느 길로 날짜가 붙든 같다.** 「매주 화요일 8시 분리수거」처럼 **되풀이 낱말**을 함께 적으면 그 회차가 지날 때 앱이 다음 회차로 옮긴다 — 매일 · 평일(월~금, 주말에 적으면 월요일부터) · 매주 · 격주 · 매월 · 매년. 낱말은 주기만 말하고 요일과 날은 나머지 글이 정한다.

| | |
|---|---|
| 큰 달 이름 옆 `‹` `›` · **오늘** | 달을 넘기고, 오늘로 돌아온다. 늘 보인다. 폰에서는 칸 밑의 점이 **메모의 색**이고(점 하나가 메모 하나, 셋까지), 고른 날의 원은 칸에서 칸으로 미끄러진다 |
...
대충 적어 두고, 다시 볼 시각만 정한다. 종이의 우클릭 메뉴 「다시 보기…」(폰은 편집 화면의 종 단추)에서 「한 시간 뒤」·「내일 아침 9시」를 누르면 **그 한 번으로 저장되고 닫힌다**; 다른 시각은 아래 피커로. **일정은 그대로다** — 회의는 3시, 다시 보는 건 2시 반. 해제하면 따로 정한 시각만 빠지고, 일정 시각이 있으면 그때 알린다고 화면이 적는다.

그 시각에 맥은 종이를 앞으로 꺼낸다. **시스템 알림은 켠 기기에서만** — 「이 기기에서 알림 받기」를 켜면 그때 한 번 권한을 묻고, 그 뒤로 일정 시각과 다시 볼 시각에 알림이 오며 누르면 그 메모가 열린다. **배너에 단추가 있다** — 길게 누르면(맥은 배너 위에 포인터) 「한 시간 뒤」「내일 아침 9시」「봤어요」, 출발 알림이면 「10분 뒤」「지도 열기」. 앱을 열지 않고 배너에서 끝나고, 미룬 시각은 파일에 적혀 다른 기기도 같은 것을 본다. 「봤어요」는 폰의 「지금」 띠와 위젯에서 그 카드를 내리고 맥은 나온 종이를 내린다. 잠금 화면에 메모 제목이 보인다는 것을 켜기 전에 적어 둔다. 날짜만 있는 메모에는 아침 알림을 지어내지 않고, 지운 것·치워 둔 것·보관한 것·완료한 것·다 체크한 목록은 걸지 않는다. **적은 직후 영수증이 선다** — 폰의 펜 아래·맥 달력 줄의 꼬리·공유 시트·Siri 의 대답에 「이 기기에 알림 예약됨 · 9월 25일 9:00」「날짜만 적혀 알림 없음」「알림 꺼짐」「알림 예약 실패 · 다시 시도」처럼 **이 기기에서 확인한 사실**만 적힌다. 알림은 이 앱이 파일을 읽은 기기에서 걸리므로 — 폰이 꺼진 동안 맥에서 적은 것은 폰이 다시 읽은 뒤에 걸린다. 양쪽에서 켜면 양쪽에서 울릴 수 있고, 집중 모드와 시스템 설정에 따라 전달이 달라진다. 서버가 없으니 기기 간 알림 동기화를 약속하지 않는다.

폰의 목록 위에는 **「지금」** 최대 세 장 — 오늘 다시 볼 것·오늘 일정·고정한 것 중에서 **다가오는 시각부터**, 그다음 방금 지난 것, 그다음 시각이 없는 것. 이유와 시각이 함께 적히고, 띠에 오른 것은 아래 「나머지」 목록에서 빠진다. 「봤어요」로 내려놓으면 이 기기에서만 사라지고, 시각을 미루거나 날이 바뀌면 다시 오른다; 끝낼 것이 있는 카드에는 「완료」가 곁에 선다. **세 장이 전부가 아니다** — 그 아래 「놓친 2 · 오늘 5 · 나중에 3」 한 줄이 나머지 전부로 가는 길이고, 지난 날에 다시 보기로 했는데 안 본 것·안 끝낸 것은 날이 바뀌어도 「놓친 것」에서 찾는다(푸시는 없다). 줄은 **오른쪽으로 밀면 완료·미루기·고정**, 왼쪽으로 밀면 보관·지우기 — 흔들면 되돌린다. 찾는 중이거나 폴더를 골랐을 때는 띠가 없다. 계획과 경계는 [docs/RECALL_PLAN.md](docs/RECALL_PLAN.md).
...
| | |
|---|---|
| 탭을 **누르기** · 메뉴바 **「서랍」** | 펼쳐서 **앞으로** 나온다 — 다른 앱이 앞에 있어도. 메뉴바로 열면 찾기 줄에 커서까지 선다. 다시 누르거나 다른 앱으로 가면 접히며 바탕화면으로 내려앉고 키보드는 쓰던 앱으로 돌아간다 |
| 탭을 **잡아 끌기** · 펼친 판의 머리 줄 | 서랍을 옮긴다. 안 끌고 놓으면 누른 것이다 |
| 탭 **오른쪽 클릭** · 메뉴바 「서랍 치우기」 | 바탕화면에서 아예 치운다. 「서랍 내놓기」로 다시 놓는다 |
...
**끝난 것은 스스로 물러난다.** 칸을 전부 체크한 목록과 「완료」한 것은 마지막으로 손댄 지 사흘 뒤, 지난 일정은 그 다음 날이 끝나면 바탕화면과 목록에서 빠진다. **지운 것이 아니다** — 파일에도 검색에도 달력에도 그대로 있고, 메뉴가 「치워 둔 N장」이라고 적어 둔다. 거기서 도로 꺼낼 수 있고, 빠른 입력에서 찾아 열어도 그 자리에서 도로 나온다. **사람의 뜻이 규칙을 이긴다** — 고정한 것, 도로 꺼낸 것(`kept:`), 칸이 남은 목록(마감이 지나도), 미래에 다시 보기로 한 지난 일정은 건드리지 않는다. 날짜가 지났다고 끝난 것이 아니다.

**완료와 보관은 사람이 적는다** (`done:`·`archived:`). 「완료」는 끝냈다는 뜻 — 알림과 「지금」에서 빠지고 사흘 뒤 물러나며 되돌릴 수 있다. 되풀이하는 일은 이 회차만 끝난 것이라 다음 회차에 도로 산다. 「보관」은 당장 안 볼 기록 — 목록에서 빠지되 검색·원문·달력은 그대로다. 「봤어요」는 이 기기에서 이 등장만 내려놓는 것이라 완료가 아니다.
...
폰에는 바탕화면이 없으므로 종이도 서랍도 없다. 있는 것은 **펜**(바닥, 적기와 찾기), **무더기**(한 목록), **달력**이다. 첫 실행의 안내는 다섯 장이고 장마다 앱의 진짜 부품(펜·「지금」 카드·줄·달력·위젯)을 작게 그린 그림이 먼저다 — More → 「사용법」으로 다시 본다. 화면의 규칙은 [`docs/MOBILE_DESIGN.md`](docs/MOBILE_DESIGN.md) 에 있다. 다른 앱에서 글을 고르고 공유 → 「lazymemo 에 적기」가 맥의 서비스 메뉴와 같은 문이고, 펜의 핀은 **누를 때만** 지금 자리를 재어 붙인다. 종이의 체크상자 `- [ ]` 는 **키보드가 내려가 있을 때 누르면** 뒤집히고(적는 중의 누르기는 커서를 놓는다), 목록 줄의 ⏎ 는 맥과 같이 머리를 잇는다.
...
| 위젯 | 무엇 | 크기 |
|---|---|---|
| **지금** | 앱의 「지금」 띠 그대로 — 오늘 다시 볼 것·오늘 일정·고정, 다가오는 것부터 세 장 (`Recall.nowCards`). 작은 것은 한 장, 중간은 세 장, 큰 것은 세 장 아래 「다음」 — 오늘 뒤에 올 일정 넉 줄까지. 큰 것은 체크리스트 카드 밑에 **아직 안 한 칸을 셋까지** 달고, 누르면 그 자리에서 체크된다 — 앱을 열지 않고 파일에 적힌다(그만큼 「다음」 줄이 준다). 카드를 누르면 그 메모가 열린다 | 작게·중간·크게 · 잠금 화면 네모·한 줄 |
...
위젯은 **앱과 같은 파일을 읽는다** — 인덱스를 열지 않고 iCloud 의 「LazyMemo」 폴더(iCloud 가 꺼진 폰은 App Group 폴더)의 마크다운만 훑는다. 그래서 맥에서 메모 폴더를 다른 곳으로 옮겨 둔 사람의 위젯은 비어 있다 — 그 폴더의 열쇠는 앱만 쥐고 있다. 화면이 바뀌는 순간(각 메모의 시각, 자정)이 시간표에 미리 적히고, 앱이 파일을 바꾸면 위젯을 다시 그리게 한다 (`WidgetRefresher`). 폰에서 「봤어요」로 내려놓은 카드는 위젯에서도 내려간다. 위젯에서 체크한 칸은 **파일에 적혀** 맥의 종이에서도 체크돼 있다 — 확장이 파일을 고치는 유일한 자리이고, 바꾸는 것은 괄호 안 한 글자뿐이다 (`WidgetChecklist`).
...
### Siri·단축어·액션 버튼 — 「lazymemo 에 적기」

앱을 깔면 문 셋이 바로 선다 — 등록할 것이 없다 (`ios/LazyMemo/AppIntents.swift`, App Intents).

| 말 | 무엇 | 앱을 여나 |
|---|---|---|
| 「lazymemo 에 적기」 · 「lazymemo 에 메모」 | 한 줄을 받아 메모로. 글을 안 주면 「무엇을 적을까요?」 하고 묻는다. 「내일 3시 치과」의 날짜와 「@강남역」의 자리를 펜과 같은 규칙(`InboundNote` → `InboxDrop`)으로 읽고, 「적었어요 · 9월 22일 15:00 · 달력으로」라고 답한다 | **아니오** — 파일 한 장을 떨구고 나온다. 위젯은 바로 다시 그려지고, 앱은 다음에 앞으로 나올 때 대조한다 |
| 「lazymemo 오늘 뭐 있어」 | 「지금」의 카드 세 장을 읽어 준다 — 앱과 위젯이 보는 그것 | 아니오 |
| 「lazymemo 펜」 | 앱을 열고 펜에 키보드를 올린다 — 말보다 손으로 적고 싶을 때의 액션 버튼 | 예 |

단축어 앱에서는 이 셋이 타일로 서고, 액션 버튼(iPhone 15 Pro 이후)에 매달 수 있다. Siri 로 적으면 말한 글은 **애플의 Siri 가 받아 적는** 동안 애플의 설정을 따른다 — 단축어에 글을 직접 넣으면 아무것도 나가지 않는다. 자리가 있는 약속을 적으면 공유 시트와 같이 「어디서 출발하시나요?」 알림 하나가 온다(알림을 켜 둔 기기에서만).
...
좌표까지 넣어 두면 그 메모는 나중에 **「적어 둔 자리에 가면 그 종이 꺼내기」에도 걸린다** — 다음에 그 근처에 갔을 때 종이가 스스로 나온다. 좌표 없이 이름만 있으면 지도 링크까지는 되고 도착 감지는 안 된다(장소 이름을 좌표로 바꾸려고 그 이름을 밖에 보내지 않기 때문이다).
...
빠른 입력의 **✦ / ⌥⌘⏎**(맥)나 펜 옆 **✦**(폰)로 비서에게 보낸다 — 기본 ⌘⏎·「남기기」는 늘 적을 뿐이다. 처음 한 번 모델(Gemma 4 E2B, 약 2.6GB)을 받으면 그 뒤로는 인터넷 없이 돈다 — 메모는 아무것도 밖으로 나가지 않는다.

- **묻기** — 「치과 언제였지?」. 답 아래에 근거가 된 메모가 붙고, 누르면 그 종이가 열린다. 메모에 없으면 없다고 한다.
- **웹에서 찾기** — 메모에 없으면 **「웹에서 찾기」**를 권한다(맥은 빈 상자에서 ⌘↵ 한 번). 「웹에서 서울 내일 날씨」「달러 환율 검색해줘」처럼 말하면 메모를 거치지 않고 바로. 검색은 앱이 DuckDuckGo 에 **키 없이** 하고(질문 한 줄만 나간다, 메모는 안 나간다), 이어서 **앞의 세 쪽 본문을 읽어 와**(쿠키 없이·8초·1MB, 못 읽으면 발췌가 근거) 모델이 그것들을 견줘 한두 문장으로 답하며 출처 링크를 단다 — 누르면 브라우저로. 첫 검색이 빈손이면 더 짧은 검색어로 한 번 더 던진다. 모델이 없으면 결과 셋을 그대로 보여 준다. 답 밑에는 **결과 전부**가 한 칸씩 선다(인용한 것이 앞, 점이 찍힌다). 그리고 **「이걸 어떻게 할까요?」** — 메모로 남기기 · 정리해서 남기기(모델) · 기존 메모에 붙이기. 글로 해도 같다: 「메모해」「정리해줘」「치과 메모에 추가해줘」(이름을 대면 그 메모에, 안 대면 고르게 한다). 남긴 메모는 **자리가 정해져 있다** — 제목(물은 말) · 답 한두 문장 · `## 핵심` 목록 · (값이 여럿이면) `## 세부` 표·번호 목록·할 일 · `## 출처` 제목과 주소 · 「어디서 찾음」 한 줄. 모델이 넘어져도 앱이 같은 자리를 발췌로 채운다. 문서화된 API 가 아니라 DDG 가 막으면 「웹에 닿지 못했습니다」다(가는 길의 네이버 지도와 같은 결정 — 키가 필요한 검색은 두지 않는다).
...
앞으로 올 약속에 자리(지도 링크·`@자리`·좌표)가 있으면 적은 직후 상자(맥)와 펜(폰)의 결과 카드가 **「가는 길 찾기」**를 권한다 — 묻지 않는다, 다음에 치는 글은 새 메모다. 누르면 **「어디서 출발하시나요?」**가 선다. 모델은 쓰지 않는다 — 앱이 정한 대화다.
...
폰의 공유 시트로 적은 약속(지도 앱의 「공유」→「lazymemo 에 적기」)은 시트에 펜이 없으므로 알림 하나로 권한다 — 「어디서 출발하시나요? 눌러서 답하면 가는 길을 찾아 적어 둘게요」. 누르면(또는 앱을 다음에 열면) 펜의 결과 줄이 「가는 길」을 권하고, 그것을 눌러야 묻는다. 시트는 적은 직후 **알림 영수증**도 한 박자 보여 준다 — 「이 기기에 알림 예약됨 · 9월 25일 9:00」「알림 꺼짐」「알림은 앱을 열면 걸려요」처럼 **이 기기에서 확인한 사실**만.
...
| 웹에서 찾기 (맥·폰) | **물은 말 한 줄** — DuckDuckGo (키·계정 없음). 이어서 결과 **앞의 세 쪽**에 접속해 본문을 읽어 온다 (쿠키 없이·8초·1MB, 물은 말은 그 사이트로 안 나간다) | 메모에서 못 찾은 뒤 「웹에서 찾기」를 **누를 때**, 또는 「웹에서 …」「… 검색해줘」라고 **말할 때만** · 메모 본문은 나가지 않는다 |
...
| Siri 로 적기 (폰) | **말한 글 한 줄** — 애플의 Siri 가 받아 적는 동안, 애플의 Siri 설정을 따른다. 메모 파일은 나가지 않는다 | Siri 에게 **말할 때만**. 단축어·액션 버튼에 글을 직접 넣으면 아무것도 나가지 않는다 |
...
- **캘린더**(읽기) — 달력 창을 **처음 열 때** 한 번. 거절하면 다시 묻지 않고, 거절했다는 사실이 설정 창에 남는다.
- **위치**(사용 중) — `⌥⌘L` 을 **처음 누를 때** 한 번. 한 번 재고 끊는다.
- **위치(항상)** — 설정에서 「적어 둔 자리에 가면 그 종이 꺼내기」를 **직접 켤 때만.** 이 앱이 요구하는 것 중 가장 무거운 권한이라 기본이 꺼져 있고, 켜 두면 **몇 자리를 지켜보는 중인지 설정 창이 적어 둔다.** 지켜보는 자리는 좌표가 이미 적힌 메모뿐이고 — 장소 이름을 좌표로 바꾸려고 당신이 적어 둔 이름을 밖에 보내지 않는다 — 도착했다는 사실만 받아 종이를 꺼낼 뿐 **위치를 저장하지도 내보내지도 않는다.**
